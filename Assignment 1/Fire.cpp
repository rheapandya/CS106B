...
void updateFire(Grid<int>& fire) {
    for (int row = 1; row < fire.numRows(); row++){
        for (int col = 0; col < fire.numCols(); col++){
            /*Creates newColumn variable (where the new random column the value will be sent)
             * and currentValue which stores the value of the current row and column
             */
            int newCol;
            int currentValue = fire.get(row, col);
            /*checks if column is not an edge column*/
            if (col>0 && col<fire.numCols()-1){
                /*randomly chooses new column*/
                newCol = randomInteger(col-1, col+1);
            }
            /*case of first column*/
            else if (col==0){
                newCol = randomInteger(col, col+1);
                /* checks if length of column is 0*/
                if (fire.inBounds(row-1, newCol) == false){
                    newCol = col;
                }
            }
            /*case of last column*/
            else if (col>fire.numCols()-2){
                newCol = randomInteger(col-1, col);
            }
            /*checks if the value of square is greater than 0*/
            if (currentValue > 0){
                /*decreases value with 2/3 probability and sets to new position*/
                if (randomChance(2.0/3.0) == true){
                    fire.set(row-1, newCol, currentValue-1);
                }
                /*doesn't decrease value and sets to new position*/
                else{
                    fire.set(row-1, newCol, currentValue);
                }
            }
            /*if value is 0 sends that value to new position*/
            else{
                fire.set(row-1, newCol, 0);
            }
